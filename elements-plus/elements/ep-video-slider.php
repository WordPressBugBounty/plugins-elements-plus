<?php
	...
	class Widget_Video_Slider extends Widget_Base {
		...
		protected function render() {
			...
			?>
			<div id="ep-vs-<?php echo esc_attr( $this->get_id() ); ?>" class="ep-slider-container" data-position="<?php echo esc_attr( ( 'below' === $position ) ? 'false' : 'true' ); ?>" data-slides="<?php echo esc_attr( $slides ); ?>">
				...
				<div class="ep-slider-item ep-slider-nav <?php echo esc_attr( $position ); ?>">
					<?php foreach ( $settings['video-list'] as $video ) {
						...
						?>
						<div>
							<div class="ep-nav-slide" style="background-image: url('<?php echo esc_url( $thumb ); ?>');">
								<h2 class="ep-nav-title"><?php echo esc_html( $video['text'] ); ?></h2>
							</div>
						</div>
						<?php
					} ?>
