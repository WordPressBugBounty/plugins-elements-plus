				...
				<ul>
					<?php foreach ( $accordion as $accordion_item ) : ?>
						<?php
							...
							$i++;
							$this->add_link_attributes( "accordion_item_$i", $url, true );
						?>
						<li class="<?php echo esc_attr( $active_class ); ?>" style="background-image:url('<?php echo esc_url( $img ); ?>');">
							<div>
								<span>
									<?php if ( 'yes' === $show_text && ( $title || $text ) ) : ?>
										<div class="ep-accordion-content">
											<?php if ( $url['url'] ) : ?>
												<a <?php $this->print_render_attribute_string( "accordion_item_$i" ); ?>>
											<?php endif; ?>
												<?php if ( $title ) : ?>
